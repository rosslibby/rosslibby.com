...
const BOOKMARKLET = "javascript:(function ts(){let k=JSON.parse([...document.querySelectorAll('script[nonce]')].find(s=>s.textContent.match(/var calendars =/)).textContent.match(/(?<=calendars: )(.*)/)[0]).map(c=>c.id);async function run(){let e=Array.from(document.querySelectorAll('[data-date]')),t=e.shift().dataset.date,a=e.pop().dataset.date;console.log(`[${t}] --> [${a}]`);let n=`events?startDate=${t}&endDate=${a}&tz=America/New_York`,i=window.location.href,d=`${i}/${n}`,{events:r}=await fetch(d).then(e=>e.json()),l=r.filter(({subcalendar_id:e})=>k.includes(e));l.map(({id:e,creation_dt:t})=>{let a=document.querySelector(`[class*='teamup-event-id-${e}']`);l.find(t=>t.id===e);let n=document.createElement('span');return n.textContent=new Date(t).toLocaleDateString('en-US',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),a?.appendChild(n),a})}run()})()";
